...
class BoardController: NSObject,
    ...
    private func applyThemeSettings(with settings: [String: Any]) {
        ...
        if let themeString = settings[kWordThemeKey] as? String {
            // Convert the theme string to WordTheme using the rawValue initializer
            if let wordTheme = WordTheme(rawValue: themeString) {
                // Use the WordGenerator to generate a new goal word based on the theme
    ...
    private func applyIsAlienWordleSettings(with settings: [String: Any]) {
        ...
        if let isAlienWordle = settings[kIsAlienWordleKey] as? Bool {
            // Update the 'isAlienWordle' property in BoardController
